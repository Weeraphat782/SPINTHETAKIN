-- Mark prizes that are not real wins (e.g. "Try Again") — no congratulations UI.

alter table prizes
  add column if not exists is_no_prize boolean not null default false;

-- Seed "Try Again!" as a no-prize outcome
update prizes
set is_no_prize = true
where name ilike 'try again%';
