-- ── RPC functions (replaces Edge Functions) ──────────────────────────────────
-- These run with SECURITY DEFINER so they bypass RLS but are called by the anon key.
-- The prize selection logic stays server-side; weights are never returned to the client.

-- Session creation: checks device replay, creates a short-lived token
create or replace function create_spin_session(
  p_nickname  text,
  p_company   text,
  p_device_id text
) returns json
...
declare
  v_token text;
begin
  if length(trim(p_nickname)) = 0 or length(trim(p_company)) = 0 then
    return json_build_object('error', 'nickname and company are required');
  end if;

  -- Already played? (skip check for test accounts containing "View" in nickname)
  if lower(trim(p_nickname)) not like '%view%' then
    if exists(select 1 from players where device_id = p_device_id) then
      return json_build_object('alreadyPlayed', true);
    end if;
  end if;

  v_token := gen_random_uuid()::text;
  insert into pending_sessions (token, nickname, company, device_id)
  values (v_token, trim(p_nickname), trim(p_company), p_device_id);

  return json_build_object('sessionToken', v_token);
end;
$$;

-- Spin: validates session, selects prize by weighted random, records play
...
begin
  ...
  -- Segment index = 0-based position in the active prizes list ordered by sort_order
  -- (must match what the wheel client renders)
  select count(*) into v_segment_index
  from prizes
  where active = true
    and sort_order < (select sort_order from prizes where id = v_chosen_id);
...
-- Grant execute to the anon role (called by the client with anon key)
grant execute on function create_spin_session(text, text, text) to anon;
